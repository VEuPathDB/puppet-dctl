...
# TODO
# service define should take $project and $env paramaters.  That would allow
# for create_resources loops in a profile to cycle through all the needed
# defines.  
# 
# It would be nice to have the defaults for url/hash/vars pulled from the
# project, and have the service ensure that the project exists first.  That
# way, we just have to define the service... Because we are expected to be
# using create_resources, we would need to make sure those paramaters are in
# the hiera structure....
...
# then lookup 'dctl::services.udis.dev' in the define?  Or use some other
# advanced resource setting (https://puppet.com/docs/puppet/5.5/lang_resources_advanced.html#)


# other non-sensicle ramblings...
# what if there were a dct::service::vars define (function?) That only returns
# the vars values?  The object here would be to have something that could be
# applied that would only return the values, and not create any resources.
# That way, it could be set in a non-root user's .pp file and applied to give
# the params of the resource.  Maybe there is a better way to pull that info from puppet.
